...
def moveit_nodes(
    ...
    pipelines_path = os.path.join(
        get_package_share_directory("openarm_bimanual_moveit_config"),
        "config", "planning_pipelines.yaml"
    )
    ...
    with open(pipelines_path, "r") as f:
        pipelines_yaml = yaml.safe_load(f)
    ...
    chomp_path = os.path.join(
        get_package_share_directory("openarm_bimanual_moveit_config"),
        "config", "chomp.yaml"
    )
    with open(chomp_path, "r") as f:
        chomp_yaml = yaml.safe_load(f)
...
def generate_launch_description():
    ...
    delayed_moveit = TimerAction(period=10.0, actions=[moveit_nodes_func])
